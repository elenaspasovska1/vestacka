...
class Explorer(Problem):
    ...
    def h(self, node):
        ...
        x_h=self.goal[0]
        y_h=self.goal[1]
        return abs(x_man-x_h) + abs(y_man-y_h)

if __name__ == '__main__':
    man_x = int(input("Enter the man's x coordinate: "))
    man_y = int(input("Enter the man's y coordinate: "))

    house_x = int(input("Enter the house's x coordinate: "))
    house_y = int(input("Enter the house's y coordinate: "))

    goal_state = (house_x, house_y)
    initial_state = (man_x, man_y)
    obstacle_1=(2, 5, -1)
    obstacle_2=(5, 0, 1)

    explorer = Explorer((initial_state[0],initial_state[1], obstacle_1, obstacle_2), goal_state)
    result= greedy_best_first_graph_search(explorer)
